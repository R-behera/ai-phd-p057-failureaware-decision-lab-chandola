...
class ProposedMethodTest(unittest.TestCase):
    def test_unique_method_runs(self):
        result = run_method()
        self.assertEqual(result["algorithm_id"], 'P057-Chandola-DataMining')
        self.assertEqual(result["algorithm_name"], 'ChandolaFailureFirstDataMiningDistillLoop')
    ...
    def test_value_add_report_includes_unique_method(self):
        report = run_demo()
        self.assertEqual(report["proposed_method"]["algorithm_id"], 'P057-Chandola-DataMining')
